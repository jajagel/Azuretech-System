<?php
		...
		$sql="select week(OrderTime) as lweek, month(OrderTime) as lmonth,year(OrderTime) as lyear, sum(Total) as totalprice from tblorderaddresses where (date(tblorderaddresses.OrderTime) between '$fdate' and '$tdate') and (OrderFinalStatus='Bottle Delivered') group by lweek";
		 ...
		 while($row = mysqli_fetch_assoc($query)){
		 	$week_start = new DateTime();
				...
				$ss=(string)$week_start->format('m/d/Y');
				$ee=(string)$week_end->format('m/d/Y');
    ...
    $pdf->AddPage();  
		...
		$html = <<<EOD
		<h3 align="center">Water Refilling Station</h3><h3 align="center">Weekly Sales Report</h3> <p align="center">Report from $m1-$d1-$y1 to  $m2-$d1-$y2</p><p></p>
		EOD;
    ...
    $pdf->Output('Weekly Sales report.pdf', 'I');
